...
#define UCASE "ABCDEFGHIJKLMBOPQRSTUVWXYZ"
#define PLUSSES "+++++"
#define MINUSES "-----"
...
main(int argc, char *argv[]){
	...
	//if we seek back to the begining of the file and
	...
	//If we again, seek back to the begining of the file and
	//write  to it, will it still append?
	rez = lseek(fd, 0, SEEK_SET);
	assert(rez == 0);
	memset(buf, 0x0, sizeof(buf));
	...
	//If we swwk past the end of th efile and write, will
	...
	//Now, let's check out the contents of the file.
	rez = stat(FNAME1, &st);
	...
	//We need to test to manke sure we didi not mess up non-append
	// write to a file
