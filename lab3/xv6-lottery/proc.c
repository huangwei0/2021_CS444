...
//# error this is an excellent place to add some new data to the o/p of cps
           cprintf("\t%u-%s%u-%s%u %s%u:%s%u:%s%u\t%u\t%u\t%u"
...
int 
renice(int new_value, int pid)
{
  int i = 0;
  if(new_value < MIN_NICE_VALUE && new_value > MAX_NICE_VALUE){
    return 1;
}
  for (i = 0; i < NPROC; i++) {
      if (ptable.proc[i].state != UNUSED) {
        if(ptable.proc[i].pid == pid){
                  ptable.proc[i].nice_value = new_value;
        }//end if
    }//end if
  }//end for
    return 2;
