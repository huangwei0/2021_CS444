...
#ifdef KTHREADS
// # error These are in here until we complete the implementation is
...
scheduler(void)
{
  ...
  struct cpu *c = mycpu();
#ifdef KTHREADS
//# error Assign the current cpu id to a variable.
...
#endif // PROC_TIMES
#ifdef KTHREADS
//# error The additional header for the kthresd information.
    cprintf("\tcpu\tthd cnt\tis thrd\tthd #");
#endif // KTHREADS
    cprintf("\n");
    for (i = 0; i < NPROC; i++) {
...
// # error However, the sz data member is probably different for each one.
...
#endif // PROC_TIMES
#ifdef KTHREADS
// # error You should only show a cpu for a process/thread when it actually
...
// # error number 1 to the characher '0'? Hmmm...
            	...
            	if(ptable.proc[i].oncpu >= 0){
            			if(ptable.proc[i].thread_count){
            				if(ptable.proc[i].is_thread){
            					cprintf("\t%d\t%d\t%c\t%d"
            					,ptable.proc[i].oncpu
            					,ptable.proc[i].thread_count
            					,'y'
            					,ptable.proc[i].tid
            					);
            				}
